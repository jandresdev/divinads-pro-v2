...
export class ClienteMetaAds {
  ...
  constructor(accessToken: string, adAccountId: string) {
    ...
    this.http.interceptors.request.use(config => {
      logger.debug({ url: config.url, method: config.method }, 'Petición a Meta API')
      return config
    })
    ...
    this.http.interceptors.response.use(
      ...
      (error: AxiosError) => {
        ...
        if (datos?.error) {
          ...
          logger.warn(
            {
              codigoMeta: metaError.code,
              tipo: metaError.type,
              mensaje: metaError.message,
              traceId: metaError.fbtrace_id,
            },
            'Error recibido de Meta API',
          )
  ...
  async actualizarEstadoCampaña(campaignId: string, estado: 'ACTIVE' | 'PAUSED'): Promise<boolean> {
    ...
    logger.info({ campaignId, estado }, 'Estado de campaña actualizado en Meta')
    return data.success
  }
  ...
  async actualizarPresupuestoAdSet(adSetId: string, presupuestoCentavos: number): Promise<boolean> {
    ...
    logger.info({ adSetId, presupuestoCentavos }, 'Presupuesto de AdSet actualizado en Meta')
    return data.success
  }
}

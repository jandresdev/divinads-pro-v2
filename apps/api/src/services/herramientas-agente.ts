...
export async function obtenerMetricasCampaña(
  ...
  logger.debug(
    { campaignId, dias, fechaLimite: fechaLimiteStr },
    'Consultando métricas de campaña para el agente'
  )
  ...
  if (error) {
    logger.warn({ error, campaignId }, 'Error al consultar daily_metrics')
...
export async function obtenerHistorialAnomalias(
  ...
  logger.debug({ campaignId, limit }, 'Consultando historial de anomalías para el agente')
  ...
  if (error) {
    logger.warn({ error, campaignId }, 'Error al consultar historial de anomalías')
...
export async function obtenerPrediccionROAS(campaignId: string): Promise<string> {
  logger.debug({ campaignId }, 'Consultando predicción de ROAS para el agente')
  ...
  if (error || !data) {
    logger.debug({ campaignId }, 'Sin predicción ML disponible para esta campaña')
...
export async function obtenerContextoTenant(tenantId: string): Promise<string> {
  logger.debug({ tenantId }, 'Consultando configuración del tenant para el agente')
  ...
  if (error || !data) {
    logger.debug({ tenantId }, 'Sin configuración de tenant — usando valores por defecto')
...
export async function ejecutarHerramienta(
  ...
  logger.debug({ nombre, input }, 'Despachando herramienta del agente')
  ...
  switch (nombre) {
    ...
    default:
      ...
      logger.warn({ nombre }, 'Herramienta desconocida solicitada por el agente')

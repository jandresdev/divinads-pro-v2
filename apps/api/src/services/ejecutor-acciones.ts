...
export async function ejecutarAccion(
  ...
  if (tipoAccion === 'solo_monitorear') {
    logger.info({ tenantId, campaignId }, 'Acción de solo monitoreo registrada — sin cambios en Meta')
    return {
      exitoso: true,
  ...
  if (!clienteMeta) {
    logger.warn({ tenantId, campaignId }, 'No hay cuenta Meta configurada para ejecutar la acción')
    return {
      exitoso: false,
  ...
  if (!campania?.meta_campaign_id) {
    logger.warn({ tenantId, campaignId }, 'No se encontró el meta_campaign_id para la campaña')
    return {
      exitoso: false,
  ...
  try {
    switch (tipoAccion) {
      ...
      case 'pausar_campaña': {
        ...
        logger.info({ tenantId, campaignId }, 'Campaña pausada exitosamente en Meta Ads y Supabase')
        return {
          exitoso: true,
      ...
      case 'reducir_presupuesto': {
        ...
        logger.info(
          { tenantId, campaignId, adSetId, presupuestoActual, presupuestoNuevo, reduccionPct },
          'Presupuesto de AdSet reducido exitosamente en Meta'
        )
      ...
      case 'aumentar_presupuesto': {
        ...
        logger.info(
          { tenantId, campaignId, adSetId: adSetIdAumento, presupuestoActual2, presupuestoNuevo2, aumentoPct },
          'Presupuesto de AdSet aumentado exitosamente en Meta'
        )
      ...
      // -----------------------------------------------------------------------
      default: {
        logger.warn({ tipoAccion, campaignId }, 'Tipo de acción desconocido recibido por el ejecutor')
        return {
          exitoso: false,
    ...
    logger.error({ error, tipoAccion, campaignId, tenantId }, 'Error al ejecutar acción en Meta API')
    return {
      exitoso: false,

...
export async function jobValidarResultados(): Promise<void> {
  console.info('[validar-resultados] Iniciando validación de resultados de acciones ejecutadas')
  ...
  if (error) {
    console.error('[validar-resultados] Error al obtener acciones pendientes de validación', error)
    return
  }

  if (!acciones?.length) {
    console.debug('[validar-resultados] No hay acciones pendientes de validación en este ciclo')
    return
  }

  console.info(`[validar-resultados] Validando ${acciones.length} acciones ejecutadas`)
  ...
  for (const accion of acciones) {
    try {
      ...
      if (errorValidacion) {
        console.warn(`[validar-resultados] No se pudo guardar el resultado de la validación: accionId=${accion.id}`, errorValidacion)
      }

      console.info(`[validar-resultados] Validación de acción completada: accionId=${accion.id} mejoro=${mejoro} roas_antes=${roas_antes} roas_despues=${roas_despues} cambioPorcentual=${cambioPorcentual}`)
    } catch (error) {
      // Error al validar una acción específica — continuar con las demás
      console.error(`[validar-resultados] Error al validar resultado de acción: accionId=${accion.id}`, error)
    }
  }

  console.info(`[validar-resultados] Ciclo de validación completado: validadas=${acciones.length} duracionMs=${Date.now() - inicio}`)
}

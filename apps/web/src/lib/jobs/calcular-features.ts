...
import {
  ...
  calcularPercentil,
  filtrarPorVentana,
} from '@/lib/services/feature-calculator'
...
async function obtenerMetricasCampaña(
  ...
  if (error || !data) {
    console.debug(`[calcular-features] Error obteniendo métricas diarias — se omitirá esta campaña: tenant=${tenantId} campaign=${campaignId} error=${error?.message}`)
    return []
  }
...
async function calcularFeaturesCampaña(
  ...
  if (!metricas.length) {
    console.debug(`[calcular-features] Sin métricas históricas — omitiendo feature engineering: tenant=${tenantId} campaign=${campaignId}`)
    return {}
  }
...
export async function jobCalcularFeatures(): Promise<void> {
  console.info('[calcular-features] Iniciando cálculo de features para todos los tenants')
  ...
  if (!campañas?.length) {
    console.info('[calcular-features] No hay campañas activas — nada que calcular en feature engineering')
    return
  }
  ...
  for (const [tenantId, campañasTenant] of Object.entries(campañasPorTenant)) {
    console.debug(`[calcular-features] Calculando benchmarks del tenant para percentiles: tenant=${tenantId} totalCampañas=${campañasTenant.length}`)
    ...
    for (const campaña of campañasTenant) {
      ...
      if (upsertError) {
        console.warn(`[calcular-features] Error al guardar features — se continúa con la siguiente campaña: tenant=${tenantId} campaign=${campaña.id} error=${upsertError.message}`)
        campañasOmitidas++
      } else {
  ...
  console.info(`[calcular-features] Cálculo de features completado: featuresSincronizados=${featuresSincronizados} campañasOmitidas=${campañasOmitidas} duracionMs=${Date.now() - inicio}`)
}

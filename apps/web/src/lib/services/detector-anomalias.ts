...
export function detectarAnomalias(
  ...
  if (roasBase7d > 0 && roasActual < roasBase7d * (1 - UMBRALES.roas_caida_minima)) {
    ...
    console.debug(`[detector-anomalias] Anomalía detectada: roas_bajo | tenant=${tenantId} campaign=${campaignId} roasActual=${roasActual} roasBase7d=${roasBase7d} caida=${caida.toFixed(1)}`)
  }
  ...
  if (cpcBase7d > 0 && cpcActual > cpcBase7d * (1 + UMBRALES.cpc_subida_minima)) {
    ...
    console.debug(`[detector-anomalias] Anomalía detectada: cpc_alto | tenant=${tenantId} campaign=${campaignId} cpcActual=${cpcActual} cpcBase7d=${cpcBase7d} subida=${subida.toFixed(1)}`)
  }
  ...
  if (ctrBase7d > 0 && ctrActual < ctrBase7d * (1 - UMBRALES.ctr_caida_minima)) {
    ...
    console.debug(`[detector-anomalias] Anomalía detectada: ctr_bajo | tenant=${tenantId} campaign=${campaignId} ctrActual=${ctrActual} ctrBase7d=${ctrBase7d} caida=${caida.toFixed(1)}`)
  }
  ...
  if (frecuencia7d >= UMBRALES.frecuencia_alta) {
    ...
    console.debug(`[detector-anomalias] Anomalía detectada: frecuencia_alta | tenant=${tenantId} campaign=${campaignId} frecuencia7d=${frecuencia7d} esCritica=${esCritica}`)
  }
  ...
  if (tendenciaConversiones < -UMBRALES.conversiones_caida_minima) {
    ...
    console.debug(`[detector-anomalias] Anomalía detectada: caida_conversiones | tenant=${tenantId} campaign=${campaignId} tendenciaConversiones=${tendenciaConversiones}`)
  }
  ...
  if (cpaBase7d > 0 && cpaActual > cpaBase7d * (1 + UMBRALES.cpa_subida_minima)) {
    ...
    console.debug(`[detector-anomalias] Anomalía detectada: cpa_alto | tenant=${tenantId} campaign=${campaignId} cpaActual=${cpaActual} cpaBase7d=${cpaBase7d} subida=${subida.toFixed(1)}`)
  }
  ...
  if (presupuestoDiarioCentavos > 0) {
    ...
    if (porcentajeConsumido >= 95) {
      ...
      console.debug(`[detector-anomalias] Anomalía detectada: presupuesto_agotado | tenant=${tenantId} campaign=${campaignId} porcentajeConsumido=${porcentajeConsumido.toFixed(1)}`)
    }
  }

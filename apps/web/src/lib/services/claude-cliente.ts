...
export async function ejecutarAgente(
  ...
  for (let iteracion = 0; iteracion < 5; iteracion++) {
    console.debug(`[claude-cliente] Iniciando iteración ${iteracion} del agente Claude`)
    ...
    console.debug(`[claude-cliente] Respuesta recibida: iteracion=${iteracion} stopReason=${respuesta.stop_reason} bloques=${respuesta.content.length}`)
    ...
    if (respuesta.stop_reason === 'end_turn') {
      ...
      if (!jsonMatch) {
        console.error(`[claude-cliente] El agente no retornó JSON válido: ${textoRespuesta}`)
        throw new Error('El agente no retornó JSON válido en su respuesta final')
      }

      console.info(`[claude-cliente] Agente Claude completó el análisis exitosamente en iteracion=${iteracion}`)
    ...
    if (respuesta.stop_reason === 'tool_use') {
      ...
      for (const bloque of respuesta.content) {
        ...
        console.info(`[claude-cliente] Agente ejecutando herramienta: ${bloque.name}`)
        ...
        try {
          ...
          console.warn(`[claude-cliente] Error al ejecutar herramienta ${bloque.name}: ${mensajeError}`)
  ...
  console.error('[claude-cliente] El agente excedió el máximo de iteraciones (5) sin terminar')

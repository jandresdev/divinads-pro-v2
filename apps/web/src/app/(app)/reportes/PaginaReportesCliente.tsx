...
import {
  FileDown, BarChart3, TrendingUp, Users, Megaphone,
...
const PLANTILLAS: PlantillaReporte[] = [
  { id: 'rendimiento_campanas', nombre: 'Rendimiento de campañas', descripcion: 'Gasto, ROAS, CTR, CPC, conversiones y CPA por campaña.', metricas: ['Gasto', 'ROAS', 'CTR', 'CPC', 'Conversiones', 'CPA'], icono: Megaphone, color: 'text-indigo-400' },
  { id: 'metricas_diarias', nombre: 'Métricas diarias', descripcion: 'Serie de tiempo de todas las métricas KPI, un punto por día.', metricas: ['Fecha', 'Gasto', 'ROAS', 'CTR', 'Conversiones', 'CPA', 'CPC'], icono: BarChart3, color: 'text-green-400' },
  { id: 'resumen_ejecutivo', nombre: 'Resumen ejecutivo', descripcion: 'KPIs totales del período con variación vs período anterior.', metricas: ['Gasto total', 'ROAS promedio', 'Conversiones', 'CPA promedio', 'CTR promedio'], icono: TrendingUp, color: 'text-violet-400' },
  { id: 'audiencias', nombre: 'Reporte de audiencias', descripcion: 'Alcance, frecuencia y engagement por segmento de audiencia.', metricas: ['Audiencia', 'Alcance', 'Frecuencia', 'Impresiones', 'CTR'], icono: Users, color: 'text-cyan-400' },
]
...
function generarCSV(datos: Record<string, unknown>[]): string {
...
export default function PaginaReportesCliente() {
  ...
  const generarReporte = useCallback(async () => {
    ...
    try {
      ...
      if (formato === 'csv') {
        descargarArchivo(generarCSV(datos), `${filename}.csv`, 'text/csv;charset=utf-8;')
      } else {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Exporta datos de tus campañas en formato CSV o JSON</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          >
            {generando ? (
              ...
              <><FileDown className="w-4 h-4" />Descargar reporte {formato.toUpperCase()}</>
            )}
          </button>
        </div>
